...
namespace GuyueBox.Core
{
    ...
    public static class ServiceManager
    {
        ...
        /// <summary>
        /// QueryServiceConfig 的输出结构。全部字段必须按序保留：本程序只取其中几项，
        /// 但字段一旦缺失或重排，后续字段就会落到错误的偏移上。
        /// 注意字符串字段声明为 IntPtr——它们指向 API 自己分配的非托管缓冲，
        /// 若声明成 string 会被封送拆收（Marshaling）提前释放，造成野指针。
        /// </summary>
        [StructLayout(LayoutKind.Sequential)]
        ...
        private static readonly object _scLock = new object();
        private static IntPtr GetSc()
        {
            // 加锁懒加载：避免多线程首次调用时重复 OpenSCManager 泄漏 SCM 句柄
            // （ServicesView 等界面可能并发刷新服务列表）。
            lock (_scLock)
            {
                if (_sc == IntPtr.Zero)
                {
