...
namespace GuyueBox.UI
{
    ...
    public static class IconPainter
    {
            ...
            SolidBrush brush = GdiCache.Brush(color);
            {
                switch (kind)
                {
                    ...
                    default:
                        ...
                        g.DrawEllipse(pen,
                            box.Left + box.Width * 0.26f, box.Top + box.Height * 0.26f,
                            box.Width * 0.48f, box.Height * 0.48f);
                        break;
                    }
            }
        ...
        private static void DrawCopy(Graphics g, Pen pen, RectangleF b)
        {
            // 后方卡片（左上偏移）
            g.DrawRectangle(pen,
                b.Left + b.Width * 0.30f, b.Top + b.Height * 0.16f,
                b.Width * 0.44f, b.Height * 0.52f);
            // 前方卡片（右下偏移，略大）
            g.DrawRectangle(pen,
                b.Left + b.Width * 0.20f, b.Top + b.Height * 0.32f,
                b.Width * 0.44f, b.Height * 0.52f);
        }

        /// <summary>保存 / 导出：向下箭头 + 底部托盘线。</summary>
        private static void DrawSave(Graphics g, Pen pen, RectangleF b)
        {
            float cx = b.Left + b.Width * 0.5f;
            float yBottom = b.Top + b.Height * 0.78f;
            g.DrawLine(pen, cx, b.Top + b.Height * 0.16f, cx, b.Top + b.Height * 0.60f);
            // 箭头朝下
            g.DrawLines(pen, new PointF[]
            {
                new PointF(cx - b.Width * 0.20f, b.Top + b.Height * 0.40f),
                new PointF(cx, b.Top + b.Height * 0.62f),
                new PointF(cx + b.Width * 0.20f, b.Top + b.Height * 0.40f)
            });
            g.DrawLine(pen, b.Left + b.Width * 0.18f, yBottom,
                b.Left + b.Width * 0.82f, yBottom);
        }

        /// <summary>排序：三行递减 + 右侧向下箭头。</summary>
        private static void DrawSort(Graphics g, Pen pen, RectangleF b)
        {
            float left = b.Left + b.Width * 0.14f;
            for (int i = 0; i < 3; i++)
            {
                float y = b.Top + b.Height * (0.26f + i * 0.24f);
                float w = b.Width * (0.56f - i * 0.14f);
                g.DrawLine(pen, left, y, left + w, y);
            }

            float ax = b.Left + b.Width * 0.82f;
            g.DrawLine(pen, ax, b.Top + b.Height * 0.26f, ax, b.Top + b.Height * 0.68f);
            g.DrawLines(pen, new PointF[]
            {
                new PointF(ax - b.Width * 0.10f, b.Top + b.Height * 0.52f),
                new PointF(ax, b.Top + b.Height * 0.74f),
                new PointF(ax + b.Width * 0.10f, b.Top + b.Height * 0.52f)
            });
        }

        /// <summary>撤销 / 恢复：逆时针环形箭头（圆弧 + 末端箭头）。</summary>
        private static void DrawUndo(Graphics g, Pen pen, RectangleF b)
        {
            float cx = b.Left + b.Width * 0.52f;
            float cy = b.Top + b.Height * 0.54f;
            float r = b.Width * 0.32f;

            // 从正上方起顺时针绕行 250°，在左上留出缺口
            g.DrawArc(pen, cx - r, cy - r, r * 2, r * 2, 270f, 250f);

            // 圆弧终点处的箭头：沿切线方向，两笔后掠形成箭头
            double a = 160.0 * Math.PI / 180.0;
            float ex = cx + (float)(Math.Cos(a) * r);
            float ey = cy + (float)(Math.Sin(a) * r);
            float dx = -(float)Math.Sin(a);   // 顺时针切线
            float dy = (float)Math.Cos(a);
            float px = -dy, py = dx;          // 切线法向
            float L = b.Width * 0.20f;

            g.DrawLines(pen, new PointF[]
            {
                new PointF(ex - dx * L + px * L * 0.7f, ey - dy * L + py * L * 0.7f),
                new PointF(ex, ey),
                new PointF(ex - dx * L - px * L * 0.7f, ey - dy * L - py * L * 0.7f)
            });
        }
