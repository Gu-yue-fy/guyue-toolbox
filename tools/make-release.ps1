<#
    ...
    Output (bin\release\):
        SysToolbox.exe   new binary (upload to the folder BaseUrl points to)
...
$exe = Join-Path $root 'bin\SysToolbox.exe'
...
Copy-Item -LiteralPath $exe (Join-Path $outDir 'SysToolbox.exe') -Force

# 4. update.json (pure ASCII safe)
$downloadUrl = if ($BaseUrl) { ($BaseUrl.TrimEnd('/')) + '/SysToolbox.exe' } else { '' }
...
[System.IO.File]::WriteAllText($shaPath, $sha + '  SysToolbox.exe', (New-Object System.Text.UTF8Encoding($false)))
...
Write-Host '  1. Upload SysToolbox.exe to your download location (must match "url" in update.json)'
